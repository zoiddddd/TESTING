...
def load_messages_from_file(filename):
	A=Path(__file__).parent/filename
	try:
		with open(A,'r',encoding='utf-8')as B:C=[A.strip()for A in B if A.strip()];return C
	except FileNotFoundError:print(f"File not found: {A}");return[]
...
def main(total_messages):
	A=total_messages;G=load_group_urls('groups.txt')
	if not G:print('No groups to process. Exiting.');return
	D=load_messages_from_file('message.txt')
	if not D:print('message.txt not found or is empty. Please create the file with your messages.');return
	B=0;E=0
	with concurrent.futures.ThreadPoolExecutor(max_workers=4)as H:
		C=[]
		while B<A or A==-1:
			for F in G:
				if B>=A and A!=-1:break
				print(f"Opening group {F}");I=D[E];E=(E+1)%len(D);J=H.submit(post_message_to_group,F,I,roblosecurity,edgedriver_path,extension_folder);C.append(J);B+=1;print(f"Message sent to group {F} - {B}/{A}")
				if B>=A and A!=-1:break
